...
namespace MissionPlanner.NewMessage
{
    ...
    public class LogList<LogEntry> : BindingList<LogEntry>, IBindingListView
    {
        ...
        private List<LogEntry> originalListValue = new List<LogEntry>();
        public List<LogEntry> OriginalList
        {
            get
        ...
        protected override int FindCore(PropertyDescriptor prop, object key)
        {
            ...
            PropertyInfo propInfo = typeof(LogEntry).GetProperty(prop.Name);
            LogEntry item;
        ...
        public int Find(string property, object key)
        {
            ...
            PropertyDescriptorCollection properties =
                TypeDescriptor.GetProperties(typeof(LogEntry));
        ...
        LogList<LogEntry> unsortedItems;
        bool isSortedValue;
        ...
        {
            // Check the properties for a property with the specified name.
            PropertyDescriptor prop = TypeDescriptor.GetProperties(typeof(LogEntry))[propertyName];

            // If there is not a match, return -1 otherwise pass search to
            // FindCore method.
            if (prop == null)
                throw new ArgumentException(propertyName +
                    " is not a valid property for type:" + typeof(LogEntry).Name);
            else
            ...
            ListSortDirection direction)
        {
            ...
            if (interfaceType != null)
            {
                ...
                unsortedItems = new LogList<LogEntry>();
                ...
                if (sortPropertyValue != null)
                {
                    ...
                    {
                        unsortedItems.Add((LogEntry)item);
        ...
        protected override void RemoveSortCore()
        {
            ...
            if (unsortedItems != null && originalListValue.Count > 0)
            {
                ...
                if (Filter != null)
                {
                    ...
                    foreach (LogEntry item in unsortedItems)
                        this.Add(item);
                }
                else
                {
                    foreach (LogEntry item in originalListValue)
                        this.Add(item);
                }
                isSortedValue = false;
        ...
        {
            this.ClearItems();
            foreach (LogEntry t in originalListValue)
                this.Items.Add(t);
            if (IsSortedCore)
        ...
        internal void ApplyFilter(SingleFilterInfo filterParts)
        {
            ...
            // the IComparable interface.
            Type interfaceType =
                TypeDescriptor.GetProperties(typeof(LogEntry))[filterParts.PropName]
            ...
            if (filterParts.OperatorValue == FilterOperator.BitMask)
            {
                foreach (LogEntry item in this)
                {
                    if (filterParts.PropDesc.GetValue(item) != null)
                    {
                        var itemValue = Convert.ToByte(filterParts.PropDesc.GetValue(item));
                        byte itemBit = (byte)Math.Pow(2, itemValue);

                        if ((Convert.ToByte(filterParts.CompareValue) & itemBit) > 0)
                            results.Add(item);
                    }
                }
            }
            ...
            this.ClearItems();
            foreach (LogEntry itemFound in results)
                this.Add(itemFound);
        }
        ...
        internal SingleFilterInfo ParseFilter(string filterPart)
        {
            ...
            PropertyDescriptor filterPropDesc =
                TypeDescriptor.GetProperties(typeof(LogEntry))[filterInfo.PropName];
            ...
            if (filterPropDesc == null)
                throw new InvalidOperationException("Specified property to " +
                    ...
                    " on does not exist on type: " + typeof(LogEntry).Name);
